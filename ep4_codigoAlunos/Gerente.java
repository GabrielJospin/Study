...
public class Gerente extends Pessoa implements InterfaceGerente{
	...
	public void cobrarTodosEmprestimos() {
		...
		int divida;
		for(int c = 0;c<numClientes;c++){
			conta = clientes[c].getValorContaCorrente();
			divida = clientes[c].getValorDaDivida();
			if(divida == 0);
			else if (divida > conta) clientes[c].pagarEmprestimo(conta);
			else clientes[c].pagarEmprestimo(divida);
		}
