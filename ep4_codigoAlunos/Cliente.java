...
public class Cliente extends Pessoa implements InterfaceCliente{
	...
	public boolean pagarEmprestimo(int valor) {
		int div = getValorDaDivida();
		int conta = getValorContaCorrente();

		if(valor <= 0 || valor > div ) return false;
		if (valor > conta){
			setValorDaDivida(div-conta);
			setValorContaCorrente(0);
			return true;
		}
		setValorDaDivida(div-valor);
		setValorContaCorrente(conta-valor);
		return true;
	}
	...
	public boolean realizarSaque(int valor) {
		...
		if(valor<= 0 || valor > conta) return false;
